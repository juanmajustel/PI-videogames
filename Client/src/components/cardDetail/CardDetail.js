...
export default function CardDetail(props) {
  ...
  const handleDeleteGame = () => {
    let answer = window.confirm(" Do you want to delete this game?");
    if (answer) {
      ...
      {detailVideoGame.name ? (
        <div className={styles.detailVista} key={detailVideoGame.id}>
          ...
          <div className={styles.detailContent}>
            <h1 className={styles.detailTitulo}>{detailVideoGame.name}</h1>
            <h4
            ...
            <div>
              ...
              {detailVideoGame.id.length > 10 && (
                <button
                  className={styles.detailBtnBack + " " + styles.space}
